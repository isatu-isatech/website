...
```ts
// "use server" — src/app/membership/apply/actions.ts
...
**File**: `src/app/membership/apply/schema.ts`

Options for `sex`, `college`, `yearLevel`, `primaryRole`, `secondaryRole` are fetched live from `Form Submissions` schema at request time via `getMembershipOptions()` (R-010). `availability` is a commitment-band enum (org decision, not Notion-sourced), `eventAttendanceWillingness` is `z.literal(true)` mandatory.
...
z.object({
  ...
  email: z.email().refine(isStudentEmail), // strict firstname.lastname@students.isatu.edu.ph, lowercased on write
  mobileNumber: z
    .string()
    .min(7)
    .max(20)
    .regex(/^[\d+\-\s()]+$/), // written verbatim as text — Notion column must be text/phone
  ...
  primaryRole: z.enum(livePrimaryOptions),
  secondaryRole: z.enum(liveSecondaryOptions),
  relatedSkills: z.string().max(1000).optional().or(z.literal("")),
  relatedExperiences: z.string().max(1000).optional().or(z.literal("")),
  availability: z.enum(commitmentBands), // Less than 2 hours … More than 10 hours
  eventAttendanceWillingness: z.literal(true), // checkbox, mandatory
...
}).refine((d) => d.primaryRole !== d.secondaryRole, {
...
| Email                        | email                             | `email` (lowercased; must be `firstname.lastname@students.isatu.edu.ph`)                                                                       |
| Mobile Number                | text                              | `mobileNumber` (verbatim text — column must be text/phone, number drops leading zeros)                                                         |
...
| Primary Role Preference      | select                            | `primaryRole` (live)                                                                                                                           |
| Secondary Role Preference    | select                            | `secondaryRole` (live)                                                                                                                         |
...
| Event-Attendance Willingness | checkbox                          | `eventAttendanceWillingness` (mandatory — schema requires `true`)                                                                              |
...
- `NOTION_MEMBERSHIP_SUBMISSIONS_DATABASE_ID` → optional fallback (per-campaign data source ID is resolved from the active campaign page; env is only for blank campaigns / local builds)
