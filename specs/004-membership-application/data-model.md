...
|               | Email                         | email                             | required, strict `firstname.lastname@students.isatu.edu.ph` (dotted alphanumeric local part, lowercased on write)                                                                                       |
|               | Mobile Number                 | text                              | required, 7–20 chars (`+`/spaces/dashes/parens allowed, stored verbatim — Notion column must be text/phone, not number)                                                                                 |
|               | Birthdate                     | date                              | required, ISO date, not future (`date:Birthdate:start`)                                                                                                                                                 |
|               | Sex                           | select                            | required — options live from Notion (`Male`, `Female`)                                                                                                                                                  |
|               | Facebook Profile URL          | url                               | optional, must be a facebook.com (or fb.com) profile link                                                                                                                                               |
...
| Availability  | Availability                  | text                              | required — commitment-band enum (`Less than 2 hours` / `2-5 hours` / `6-10 hours` / `More than 10 hours`; org decision, not Notion-sourced; written as text)                                            |
|               | Event-Attendance Willingness  | checkbox                          | required — schema is `z.literal(true)`; the box must be checked to submit                                                                                                                               |
...
- `Mobile Number` was `number` (`FLOAT`) at verification time — changed to `text` (officer action) because number columns drop leading zeros and `+63`.
