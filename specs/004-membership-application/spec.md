...
- **FR-007**: Birthdate MUST be a real calendar date not in the future; Email MUST be an ISAT-U student address (`firstname.lastname@students.isatu.edu.ph`, dotted alphanumeric local part, case-insensitive, stored lowercase); Mobile MUST be a plausible phone value (7–20 chars, stored verbatim as text so leading zeros survive); Facebook Profile URL (when provided, optional) MUST be a facebook.com profile link; Availability MUST be one of the org-defined commitment bands.
- **FR-008**: Both consents (privacy notice; declaration of accuracy) MUST be explicitly given — unchecked consent blocks submission. Event-attendance willingness is likewise mandatory (`z.literal(true)`) — an unchecked box blocks submission.
...
- **Storage destination — verified**: The `Membership Application Dashboard` page (`https://app.notion.com/p/3c7f42d3fa7280d286adddcc19b555e0`) is the central store for all academic years. It hosts `Membership Campaigns` (`collection://3c7f42d3-fa72-8095-b5a7-000bc5bec8d2`) and, via campaign template `A.Y 2XXX-2XXX`, a **per-campaign** `Form Submissions` database — a new data source is generated for each campaign (e.g., `A.Y. 2026-2027` → `collection://023f42d3-fa72-8224-9e09-87222c7d3ae6`). `Membership Campaigns` verified schema: `Academic Year` (title), `Campaign ID` (auto_increment), `Status` (Draft/In progress/Closed). `Form Submissions` verified schema includes `Student ID` (title), `Full Name` (text), `Email`, `Mobile Number` (number), `Birthdate` (date), `College`/`Year Level`/`Sex`/`Primary`/`Secondary Role` (selects with live options), `Program`, `Related Skills`/`Experiences`, `Availability` (text), `Event-Attendance Willingness` (checkbox), `Campaign` relation, etc. **The submissions DB ID is not static — it is resolved at runtime by querying the active `In progress` campaign and extracting its inline `Form Submissions` data-source ID from the campaign page's content** (fallback to `NOTION_MEMBERSHIP_SUBMISSIONS_DATABASE_ID` env var for blank campaigns). Actual types may differ from the earlier `specs/001` draft — this spec defers to the live schema as source of truth.
...
- **Dependencies**: (1) `Campaign` relation property on each per-campaign `Form Submissions` DB (linking to `Membership Campaigns`) must exist (already present on template-generated DBs); (2) an active campaign (`Status = In progress`) created via the dashboard template (which auto-generates its per-campaign `Form Submissions` DB) must exist before the form can accept production submissions (seed `A.Y. 2025-2026` `In progress` for QA, now `A.Y. 2026-2027` `Draft` with DB `023f…`); (3) `NOTION_MEMBERSHIP_CAMPAIGNS_DATABASE_ID` is required validated setting (env, `.env.example`) per P4 — submissions DB ID is **dynamic per active campaign** (extracted from campaign page content, fallback to optional `NOTION_MEMBERSHIP_SUBMISSIONS_DATABASE_ID` env var for blank campaigns/local builds); (4) org-supplied copy for confirmation/"what happens next" before those surfaces ship.
