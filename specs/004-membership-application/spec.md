...
- **FR-007**: Birthdate MUST be a real calendar date not in the future; Email MUST be an ISAT-U student address (`firstname.lastname@students.isatu.edu.ph`, dotted alphanumeric local part, case-insensitive, stored lowercase); Mobile MUST be a plausible phone value (7–20 chars, stored verbatim as text so leading zeros survive); Facebook Profile URL (when provided, optional) MUST be an http(s) URL; Availability MUST be one of the org-defined commitment bands.
