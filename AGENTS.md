...
- `src/components/` — UI: `ui/` (Radix-based primitives), `home/` (homepage sections), `layout/` (header/footer), `common/` (shared utilities like `optimized-image`, `error-boundary`), `seo/`, `three/` (R3F scenes), `assets/` (SVG logos, social icons; heavy raster art lives in `public/assets/` + `mask-*` utilities, not as components).
- `src/lib/` — `notion/` (Notion API client + helpers), `services/` (feature services), `quiz/` (quiz data + canonical-role helpers + session progress), `constants/` (site config + design tokens), `hooks/` (custom hooks), `env.ts` (Zod-validated env), `utils.ts` (`cn`).
...
- **Charts/3D-heavy assets:** Heavy raster art lives in `public/assets/` (decorations, textures, adviser photos) and is applied via `mask-*` utilities in `globals.css` — do not re-inline SVGs as React components. Remaining heavy file is `src/components/three/lanyard.tsx` (dynamic `ssr:false` + reduced-motion guard) — edit carefully and avoid reformatting wholesale.
...
- **Quiz OG banner (spec 003):** `/api/og/quiz` renders only the 17 canonical outcomes (derived from `Object.keys(archetypes)` in `src/lib/quiz/data.ts` via `src/lib/quiz/canonical.ts`); non-canonical roles 302 to the invite banner. Bundled Poppins TTF lives in `src/app/api/og/quiz/fonts/` (Satori rejects woff2 — keep TTF/OTF). No Vercel KV anywhere: the site is KV-free since 003; contact rate limiting is cookie-based (`src/lib/services/cookie-rate-limit.ts`).
- **Quiz progress (FR-008):** in-progress quizzes persist to `sessionStorage` (`4h-quiz-progress-v3` via `src/lib/quiz/progress.ts`) — refresh/back-forward resumes; discarded when the tab closes.
...
- **4H glossary:** "Hound" is canonical (no "Hypeman" anywhere). `COLORS.quiz` in `design-tokens.ts` is the single source for archetype colors (hex `from`/`to` plus literal Tailwind `gradient`/`text` classes for the scanner) — keep the pair in sync, never build dynamic `from-[...]` classes.
