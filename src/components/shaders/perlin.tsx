...
const TextureMesh = ({
  ...
  const fragmentShader = `
        ...
        uniform float u_size; //units:px, min:2, max: 128, step:1
        uniform float u_factor; //min:0,max:32
        uniform float u_gain;
        uniform int u_octaves; //min:1, max: 8
        uniform float u_phase; //units:%
        uniform float u_lacunarity; //min:1, max: 32
  ...
  // We use `useMemo` to ensure the shader material is only created once.
